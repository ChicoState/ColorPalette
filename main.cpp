...
int main(int argc, char *argv[])
{
  ...
  for (int i = 1; i < argc; i++)
  {
    ...
    if (length < 3)
    {
      cerr << argv[i] << " ERR_MISSING: One or more RGB values should be provided as arguments, separated by spaces\n";
      return 1;
    }
...
bool all_hex(char *word)
{
  for (int i = 0; i < strlen(word); i++)
  {
    ...
    {
      cerr << word[i] << " ERR_VALUE: RGB values must be in 0-F range\n";
      return false;
    }
  }
